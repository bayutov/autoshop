class ImageUploadH:
    ...
    @classmethod
    def get_field_to_combine_and_upload_postfix(cls, klass): #получение имени поля и постфикса
        field_to_combine = cls.FIELD_TO_COMBINE_MAP[klass]['field']
        upload_postfix = cls.FIELD_TO_COMBINE_MAP.get("upload_postfix", cls.FIELD_TO_COMBINE_MAP['defaults']["upload_postfix"])
...
def upload_function(instance, filename):
    ...
    field_name_to_combine, upload_postfix = ImageUploadH.get_field_to_combine_and_upload_postfix(instance.__class__.__name__)
    image = ImageUploadH(field_name_to_combine, instance, filename, upload_postfix)
